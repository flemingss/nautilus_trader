...
- **A candidate worth deploying** - stage 02, nine rows and one decision. Attribution has
  answered the premise question for the gap-fade family; what to do with the family is the
  owner's, and the rows that refine it wait on that answer.
...
| **Reject the gap-fade family, or revise it into a premise that is not long the market** | 02     | Attribution measured 2026-09-10 ([ADR-0026](decisions/0026-attribution-is-measured-per-trade.md), `strategies/out/attribution_20260911T012725Z.json`): across twelve walk-forwards and the pool, no four-factor alpha above zero under either exit-session treatment, and five results below zero under both. The pool's gross return is its exposure, and AAPL next-close - the one walk-forward whose interval cleared zero - is +0.112 R of factor exposure against -0.011 alpha at beta 1.16. **Recommendation: reject the family as a source of edge**, and if the gap signal is kept, revise it as a market-neutral experiment (long the fade, short the index over the same window) so that attribution has nothing to charge it for. Either way the pooled holdout stays unspent. Revisit trigger: a revised premise whose walk-forward shows alpha under both treatments. |
...
| **Revise the AAPL next-close premise as a new experiment**                             | 02    | Owner decision 2026-09-05: `revise`. The holdout is spent, so a revision is a new activation with a new premise and no holdout of its own on this series (ADR-0014). What to change is the research question: the edge was a 54.1% win rate with the average loss larger than the average win, underwater by more than it made, crossing zero at USD 25,000. **Sharpened 2026-09-10 by ADR-0024**: recomputed under the interval gate the same numbers return `insufficient_evidence`, not `pass` - +0.035130 R with a 90% interval of [-0.140, +0.195] R on the net series and 102 effective trades (first filed as [-0.126, +0.209], which was the gross series). The premise was never disproven, so the revision to reach for is the one that raises the effective sample rather than the one that changes the rule: pooling across the nine registered ETFs is the row below, and it is the same work.  |
| **Build the pooled holdout spend, for a premise worth spending it on**                 | 02    | Owner decision 2026-09-10: **exclude AAPL**, whose window was viewed at its spend (ADR-0014); SCHX's voided spend left its window unviewed and the other seven were never spent. **When to spend is the open half, and the answer is not on this family**: attribution finds no alpha in the pooled premise (ADR-0026), and the pooled window is the largest single-use evidence left. `pool` still refuses a spend and names the exclusion; building the spend waits on a premise worth it.                                                                                                                                                                                                                                                                                                                                                                                                                 |
