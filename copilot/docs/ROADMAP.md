...
- **The paper VM and unattended running** - stages 06, 07, 08 and 09. The nine prep items in
  ...
  Batch B is what unattended running needs; IB Gateway headless is the one prep item only the VM
  can close.
...
Fifty-two items. Grouped by blocking condition rather than by component, because that is
...
open with batch B, in the order to take them.
