...
| **Feed the live node its daily bar**                                                                                         | 08    | **Nothing else blocks a paper session that decides anything, and this was not tracked.** `GapReversalStrategy.on_start` subscribes to `1-DAY-LAST-EXTERNAL`; in the adapter (`crates/adapters/interactive_brokers/src/data/core.rs:1302`) any spec other than 5 seconds routes to `reqHistoricalData`, and IB refuses US equity historical bars on this account with **2188** - re-probed 2026-09-04 on REALTIME and DELAYED, SMART and directed, all five shapes. So a paper session would connect, warm up, and never receive a bar: **zero decisions, reading as "no setups triggered"** - the same failure shape as the unwarmed strategy. Two routes: buy consolidated data (blocked on the equity minimum), or **publish the catalog's own bars from an actor**, which is free, uses the series research scored, and makes the live-versus-replay comparison a comparison. `LiveNode.add_actor` is exposed on this build. |
