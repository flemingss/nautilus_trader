...
| **Freeze a candidate and spend the holdout**                     | 02     | Track A has never finished. `spend_holdout` exists and [ADR-0014](decisions/0014-the-holdout-is-spent-as-one-more-fold.md) says how, but no candidate has been declared frozen, so the single-use out-of-sample estimate is unspent and **every paper stage below is machinery waiting on a premise that has not passed its last gate**. Gated by the repin decision above, and in that order: the holdout spent under one cost model is not evidence for another, and re-running it is spending it twice - the same argument [ADR-0012](decisions/0012-the-holdout-is-carved-at-2022-01-01.md) made for entry timing. |
