...
| **Pool the gap fade across the registered universe**                              | 02    | The remedy ADR-0024 points at for `insufficient_evidence`, and the owner concurred 2026-09-10. Nine symbols share one premise and one parameter set, scored as one experiment, so the effective sample grows without touching a spent holdout. Survivorship is largely absent - the universe is broad ETFs and mega-caps chosen for liquidity, not for having survived. Expect it to retire the premise rather than rescue it: `cost_impact` already reports two of three symbols with no edge since 2021, and pooling makes that visible in one number instead of nine.                                                                                                                                                                                                                                                                                                                                     |
...
| **Stop `--pull --dataset` from writing one venue's symbols into another's store** | 00    | Found 2026-09-09 while repairing TLT. `--pull` routes each catalog symbol to its listing venue's dataset; passing `--dataset` overrides that for the routing but not for the symbol selection, so every venue's symbols are fetched against the named dataset and written into its directory, one batch overwriting the next under the same filename. The store briefly held ten NYSE names under `XNAS.ITCH`, and nothing complained - `patch` still reported *no source*, which is what exposed it. A pull that names a dataset should either restrict itself to that dataset's symbols or refuse. `--only` is the safe form meanwhile.                                                                                                                                                                                                                                                                    |
...
| **Switch the commission structure, and remodel it**                               | 02    | IBKR **Lite is disqualified**: it has no API access, which this whole system is. Inside Pro the choice is Fixed against Tiered, and the paper account is measurably on Fixed - the stage-five trips paid USD 2.01 and 2.02 on one and three shares, the USD 1.00 per-order minimum twice. Tiered's minimum is USD 0.35 plus pass-through exchange and regulatory fees, so the round trip at USD 20 risk moves from 0.100 R to roughly 0.04 R against an AAPL holdout edge of 0.035 R. The shakedown's `round-trip` step measures it rather than assuming it. Changing `COMMISSION_MIN` moves every verdict's cost basis, so this needs an ADR and a revalidation pass, the way ADR-0011 and ADR-0019 did for spread.                                                                                                                                                                                         |
