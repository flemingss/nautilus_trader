...
| ~~Check the holdout can be evaluated before spending it~~ **Done 2026-09-04**                                                | 02    | `spend_holdout` now selects on development and projects the holdout's trade count before touching a holdout bar, refusing below the scorer's floor. Measured against both real spends: AAPL projects 110.7 against the 111 it produced. It does **not** catch SCHX's 34.3 against 4 - that window was long enough and quiet, which is a regime difference no earlier bar can predict.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **Decide whether an unscorable spend consumes the holdout**                                                                  | 02    | SCHX's holdout is spent on a result that scored nothing: `insufficient_test_trades: 4 < 5`. No evidence was extracted, so arguably nothing was spent - but letting it be re-spent is also what would let a boundary be moved until the trade count is convenient. Owner's call; it changes what ADR-0014 means.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ~~Derive the risk budget from account equity~~ **Done 2026-09-04**                                                           | 06    | `run_activation` reads the account's USD total once the exec client is up, derives `R = A * r` and the 10% notional cap from `playbook/RISK.md`, and hands both to the strategy through `size_against` before the decision bar. `--allocation` sizes against less than the account, never more. Run live: equity 1,000,253.87 reported, sized against 1,000, risk budget 1.00, cap 100.00 - every number in the session record. Not modelled, deliberately: the settled-cash term (the paper account cannot exercise it) and the gap allowance `g` (it would move every verdict).                                                                                                                                                                                                                                                                                                      |
| ~~Cap exposure across activations, not just within one~~ **Done 2026-09-04**                                                 | 06    | `risk/exposure.py` is the playbook's two session-wide caps - total open planned risk (0.50%) and daily new entries (2, a declared canary default) - as a ledger every strategy in the node reserves against before it builds an order, and releases when the position closes or the entry is denied, rejected or cancelled. `run_activation` now runs the basket in **one node** with one ledger; strategies that cannot see each other's reservations cannot be capped together. Run live: nine activations, `0 of 5.00 reserved, entries 0/2`, each with its basket position in the record.                                                                                                                                                                                                                                                                                          |
...
| ~~Make onboarding a symbol one command~~ **Done 2026-09-04**                                                                 | 00    | `python -m copilot.data.onboard --symbols SCHX.ARCX --survey` reports where each symbol stands across five ordered stages and what the next step is. It is a status report before it is an executor, so re-running it is the record of how far the process got. Running it immediately found that SPLG is not uncovered as first concluded - its series stops on 2026-07-17, which a narrow probe reads as absent and a wide one reads as fine.                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| **Run the live path on validated parameters, not the seeded identity**                                                       | 08    | `run_activation` passes only the activation's `[parameters]`, so the live strategy uses `min_gap_atr = 0.25` from the strategy default while selection chose 0.40. Nothing is frozen yet so nothing is wrong today, but the live decision is not the researched one, and the replay comparison would compare against a rule the gate never scored.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ~~Cover every registered instrument in the preflight~~ **Done 2026-09-04**                                                   | 08    | `preflight` resolved a hard-coded `AAPL, MSFT, SPY` and reported `instruments_resolved PASS 3` over a registry holding nine. Six newly onboarded instruments reached the evening unverified behind a green line. It now derives the list from the activations and passes 9/9.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| ~~Answer "if anything changed" before spending two minutes on it~~ **Done 2026-09-04**                                       | 08    | Every verdict now carries the four digests it was computed from - data inside the window, identity, cost basis, code as it stands in the working tree - and `validate --changed` skips any activation whose newest filed verdict matches, naming the input that moved when it does not. The append lands past the window, so it cannot change the data digest; that is the property that makes the morning cheap. `onboard` reads the same digests, so "validated" now means current, not merely filed.                                                                                                                                                                                                                                                                                                                                                                                |
