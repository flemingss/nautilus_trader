...
| **Append the catalog daily** | 08 | Both halves need it and neither has it: the warm-up refuses a catalog older than the last session, and nothing runs `backfill` on a schedule. The same missing limb - something that runs unattended on a clock - is what the alerting row needs, so they are one piece of operational work wearing two hats. Now unblocked: the 2026 repair landed, and a rejection is known to be routine rather than exceptional, so the appender has to survive one rather than stop on it. |
