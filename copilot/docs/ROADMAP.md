...
| 08  | Live deployment      | Nautilus `LiveNode`           | **Supervised: ready on the dev box.** VM package due 2026-09-16; unattended gaps open |
...
- **The paper VM and unattended running** - stages 06, 08 and 09, with a package due
  2026-09-16. New: the VM's bootstrap and timers, IB Gateway headless, a heartbeat, and the
...
Twenty-seven items. Grouped by blocking condition rather than by component, because that is
...
| **Write the paper VM's bootstrap, services and environment template**                  | 08    | Target: a package ready by 2026-09-16, when the owner stands up a hand-configured Ubuntu VM ([ADR-0022](decisions/0022-the-always-on-host-is-a-dedicated-vm.md)). In `copilot/ops/` per [ADR-0006](decisions/0006-ops-progression.md) - roles in the repository, instances outside it: a bootstrap runbook drawn from *Standing up a new machine*, systemd units and Eastern-anchored timers for `day morning`, `day evening`, `day sweep` and the shakedown battery, an environment template naming every variable and no secret, clock synchronisation, and log and record retention. Size the VM for the source build: today's OOMs were on 15 GB.                                                                                                                                                                                                                                                        |
| **Run IB Gateway headless on the VM**                                                  | 08    | Never yet run against Gateway; every session so far went through TWS on the Windows host. ADR-0006 names the IBC-based `gnzsnz/ib-gateway` image. Known traps to carry in: its `read_only_api` defaults to true, which blocks the execution client with IB 321; the paper port is 4002 (`GATEWAY_PAPER_PORT`), and `session.py`'s two-check paper guard has to agree with it; the daily restart is the normal case, so the first week is a restart drill; and the `IBAPI_TIMEZONE_ALIASES` workaround exists for a JST-configured TWS and has to be re-measured on an Eastern-configured Gateway. **One session per login**: once the VM streams, the dev box reaches the broker through the VM's Gateway - distinct client ids over an SSH tunnel - rather than a second login.                                                                                                                             |
