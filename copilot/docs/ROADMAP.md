...
wholly below zero. Nothing is frozen, so stage 08's supervised readiness has nothing to
deploy.
...
- **A candidate worth deploying** - stage 02, ten rows. The pooled premise's remaining
  questions and attribution, which the filed trades now unblock.
...
| **Measure attribution against market and style factors**                               | 02    | No model involved and nothing new to collect but factor returns, for which Ken French's library is free. Regress each strategy's return series against the market and a small set of style factors and find out whether anything survives. It is a free read on every premise already measured and it answers a live question: the AAPL holdout returned 0.035 R at a t-statistic of 0.35 with exposure at 0.66, and nobody has asked how much of that residue is beta. Surfaced by [`DRAFT_ASSISTED_DECISIONS.md`](DRAFT_ASSISTED_DECISIONS.md), useful independently of it. **Unblocked 2026-09-10**: every verdict, pooled verdict and holdout record now files its scored trades with their dates and round-trip cost as `trade_rows`, so the regression reads a record rather than a replay. The sharper question is the pool's: +0.067 R per trade gross, 0.037 of it cost, an interval straddling zero - if the gross is beta, the premise is finished without spending anything. |
