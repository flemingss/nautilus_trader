...
**All three were decided on 2026-09-11**, the owner concurring with each recommendation.
...
| Item                                                                   | Stage | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------------------------------------------------------------- | ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Build the random-entry null control the research playbook requires** | 04    | Opened 2026-09-11 by the next-premise groundwork. [`RESEARCH.md`](playbook/RESEARCH.md) lists *a randomised or permuted signal with similar trading frequency* among every experiment's baselines and *benchmarks and null controls run* on its checklist; no code builds it. For a timing premise it is the only test that can tell chosen windows from chance, because ADR-0026's attribution books timing to the market loading. Shape: the same instrument, trade count, holding lengths, stop and sizing, with entry dates drawn at random from the development window, replicated, and the premise's mean read against that distribution. Filed beside the verdict, not instead of it.                           |
| **Write the experiment card and build the turn-of-month premise**      | 02    | Decided 2026-09-11, after the null control above it. The card comes first and is written before any backtest runs ([`RESEARCH.md`](playbook/RESEARCH.md)): the hypothesis, the window (the last session of a month through the third of the next), the market-on-close entry that does not depend on the price it fills at, the protective stop the sizing needs, the declared effect size, and the advance gates. Then the strategy, an activation for SPY with its own holdout boundary, the walk-forward, the null control beside it, and the interval. Its own evidence arithmetic says about 200 trades cannot show an edge below roughly 0.1 R; the card should say what it expects and why that clears the bar. |
