...
| **Check the holdout can be evaluated before spending it**                                                                    | 02    | The SCHX spend on 2026-09-04 returned FAIL on `insufficient_test_trades: 4 < 5`. The holdout was structurally incapable of producing a verdict and the spend is single-use, so the evidence was destroyed to learn that. The trade count is estimable from the development window's rate before anything is spent, and `spend_holdout` should refuse rather than burn a holdout it cannot score. |
