...
| **Recompute every verdict under Tiered and the gap allowance** | 02, 04 | **The account moved to IBKR Pro Tiered on 2026-09-12** and the pin moved with it, which closes the owner's half of ADR-0025. Two changes now sit between the filed verdicts and the code that would reproduce them, and both change every R: the commission plan, and the stressed gap allowance built the same day. So the 28 filed verdicts, the two pooled runs, the attribution and the turn-of-month premise are all in a unit the code no longer computes. One re-file carries both - `validate --all --write` recomputes from the moved cost digest, then the pool, the attribution and `null_run`. **No verdict may be quoted until it is refiled.** ADR-0025's revalidation predicted 0 of 12 majorities change on the plan alone; the allowance is the one that could move a reading, because it shrinks every position and rescales every R. One empirical check is still open: the next shakedown round trip should pay about USD 0.35 plus the pass-through rather than the USD 1.00 minimum twice. |
