...
| **Derive the risk budget from account equity**                                                                               | 06    | `run_activation` passes the activation's `risk_budget = 1000` to the live strategy verbatim. On a USD 1,000 account that sizes as if risking the whole account per trade - roughly 67x the account in notional on an $88 ETF - and `max_notional_per_order` is inert on IB, so nothing downstream catches it. `account_sweep` already does the equity-to-budget conversion offline; the live path has never been wired to it.                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
