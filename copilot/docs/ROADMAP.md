...
of [-0.140, +0.195] on the net series. The pooled nine-symbol walk-forward passes its majority
gate and **does not clear zero**: +0.030 R per trade net, interval [-0.018, +0.079]. Its fold
detail puts what edge there is in the era before the pool was a pool: 12 of 22 three-symbol
folds at +0.080 R against 5 of 11 folds of seven or more symbols at +0.015 R. Of the twelve
per-activation walk-forwards, eight pass their majority and one has an interval clearing zero:
AAPL next-close, whose spent holdout then could not. EEM, HYG and TLT have intervals
wholly below zero. **And attribution finds no alpha anywhere**
([ADR-0026](decisions/0026-attribution-is-measured-per-trade.md)): no result has positive
four-factor alpha under either treatment of the exit session, five have negative alpha under
both, and the pool's gross +0.067 R per trade is +0.017 cash, +0.051 factor exposure and
-0.001 alpha. The gap-fade family is long the market and paid for it. Nothing is frozen, so
stage 08's supervised readiness has nothing to deploy.
...
Fifty-six items. Grouped by blocking condition rather than by component, because that is
...
premise's holdout, D hygiene. The first seven rows here are batch A and the start of B, in
...
| **Re-read the latch inside a running node**                          | 09     | The latch is read once, at build, so `kill` cannot stop a session already running: its engine stays `ACTIVE` until the process exits, and the cancel limb is the only immediate effect. The runbook advertises *stop all trading on this host*. A clock timer in the nodes `build_paper_node` builds that halts the engine when the latch appears, or the runbook says what `kill` does to a running node.                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **Make the flood guard span processes**                              | 09     | `FloodGuard` keeps its state in memory and every unit is a oneshot process, so nothing is ever deduplicated on the VM: the *halt latch still engaged* WARNING fires from every phase, every day. A small on-disk window beside the receipts log, or drop the guard and say repeats are the design.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **Give the day units `OnFailure=` and an ordering**                  | 08     | A unit that dies for a reason the code did not anticipate - a traceback, an OOM kill at `MemoryMax`, the 45-minute `TimeoutStartSec` on a slow vendor day - notifies nobody; the failure is a journal line. `OnFailure=` to a unit that alerts; `After=docker.service` and a dependency on the Gateway container so a reboot at 08:25 does not hand the 08:30 evening a Gateway still logging in; a timeout sized to the sweep's worst case.                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **Partition client ids per module**                                  | 08     | `run_activation` and `failure_injection` both default to 871/872; `order_types` and `strand_recovery`'s second session to 841/842. Only the timers' spacing keeps them apart, and an overrun makes IB refuse the second connection with no clear message. One table of client-id pairs, one per module, asserted unique by a test.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **Lock the latch and the receipts log**                              | 09     | Every engage stages through the same `HALT.tmp`, and nothing locks the receipts log or `live/out/`, so two processes engaging or settling at once can interleave: the id printed may not be the id on disk, and a receipt can be resolved twice. Low impact today; a lock file costs a few lines.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **Hold the ops package to the code with tests**                      | 08     | Nothing reads `env/*.example` against `required_environment`, so renaming a variable breaks the VM silently; `install-units.sh` has no test; the timers are tested only for containing `America/New_York`, not for their times against the runbook's table or `Persistent=`; the compose file only for its digest, not for `TRADING_MODE`, `READ_ONLY_API` or its ports staying on localhost; and no test says what `day` does under an engaged latch.                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **Assert every order-capable node is built by `build_paper_node`**   | 09     | ADR-0027's own consequence: the latch is only as good as that rule, and a future builder that adds an execution client bypasses the switch. A test that finds execution-client construction outside the builder, or a builder that is the only import of the client.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **State the exit-session bracket as an agreement test**              | 04     | ADR-0026 and `attribution.py` say including the exit session flatters alpha, so *included* is the upper bound. The filed output disagrees for AAPL next-close: -0.026 included, +0.001 excluded. `bracketed_verdict` is an agreement test, not a signed bracket, and the pooled result's `bracket_disagrees` is its headline. Say so in the module and note it against the ADR.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Label the pool's two means**                                       | 04     | The pooled record files `mean_oos_net_r` 0.060 - a mean of 38 fold scores - two keys above an `evidence.mean_r` of 0.030 per trade, and the CLI prints the first above the interval of the second. `validate.py` carries the explanatory comment; `pool.py` and its record do not. Name the fold mean for what it is, or drop it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **Hold the pool's membership constant at both ends**                 | 04     | The filed pool is three symbols for 22 of 38 folds, then seven to nine, then four at the tail because each member stops at its own holdout boundary, which `from_year` cannot fix. Its 20 of 38 majority equal-weights a four-symbol 21-trade fold with a nine-symbol 157-trade one, and `majority: true` sits beside `clears_zero: false` with nothing reconciling them. The next premise's pool needs a constant membership window, or two verdicts.                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **Say the thirteen attributions are not thirteen tests**             | 04     | The pool contains every per-symbol record's trades, so the twelve verdicts and the pool are one body of evidence read thirteen times, at 90% each, under two exit treatments. ADR-0026 reads *five negative* as corroboration. State the overlap in the record; no multiplicity adjustment is needed to reject, and one is needed before anything is ever called alpha.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **Document the attribution floor and survive a degenerate resample** | 04     | `MIN_TRADES_FOR_ATTRIBUTION = 30` is six observations per coefficient for a five-parameter fit, before any dependence discount, and ADR-0026 does not mention it. At that size a near-singular block resample raises `DegenerateDesignError` from inside the 2,000-replicate loop and aborts the whole attribution rather than discarding the draw.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **Score the trade open at a fold boundary**                          | 04     | `walkforward` replays to `fold.test_end` and reads `positions_closed()`, so a position still open at the boundary appears in no fold - its signal precedes the next fold's start. About one trade per fold, uncounted, and a loss cut at the boundary is the kind the interval should see.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **`signal_created_at` is the fill, not the signal**                  | 04     | `nautilus_replay` sets it to `opened_at`; for `next_close` the decision is a session earlier. Harmless for attribution and the bootstrap, which key off the open, but `types.py` documents the field as the bar that produced the signal, and an audit of an entry against its origin would read the wrong bar.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Small defects in attribution and evidence**                        | 04     | Each confirmed, none triggered by a filed run: `attribute` traces back on a premise whose trades all exit the next session (every regressor zero, `DegenerateDesignError` uncaught); a same-session round trip is diagnosed as a factor vintage too old; an interval over fewer than two trades files `lower_r` above `upper_r` and `clears()` any negative bar; an interior factor session missing from one vintage is dropped silently; `exposures` is hardwired to the four-factor set so `MODELS` cannot hold another; the R denominator is refused at zero in one place and counted in three.                                                                                                                                                                                                                                                                                |
| **Test the validation paths the real runs exercise**                 | 04     | The last-block truncation in `assess` and `_bootstrap` is untested on a non-divisible count, and every real run is one; nothing asserts attribution's `mean_r` equals the verdict's score (`require_describes` is wired only into the spend); the clustering tests use a synthetic series shaped like the one the trade-index statistic can see; no test at the n=30 floor, of a duplicate row, or of a single-session premise.                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Apply the retention rule to `calibration/out/`**                   | 10     | `account_sweep_20260903T131926Z.json` and `spread_snapshot_20260831T173204Z.json` are named by no module, ADR, playbook or roadmap row, against the rule in `MAINTENANCE.md`. Cite each where its numbers are used, or prune it in a filing commit.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **Docs and register hygiene**                                        | -      | The roadmap's *Detail* sections still say no paper node exists, the paper run is not started and the holdout is unspent - true when written, stale now, and a reader who reaches them cannot tell. Date each detail section or fold what still matters into the tables. `python/pyproject.toml` carries a two-line comment that #83 split across the `shakedown.py` entry; rejoin it, registered file though it is.                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| **Wire the protection guard into the basket, with its ledger**             | 06    | Found 2026-09-10 while reviewing its cooldown: `ProtectionGuard` is not added to any node, so no live run has ever had the account-wide breaker. Inert while orders are denied; required before paper stage seven enables them. Configure it with `ledger_path=risk_ledger_path(account)` so its evidence survives the one-process-per-step day, and take the risk engine handle before start.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
