...
| 02  | Research / strategy  | `copilot/strategies`          | **NOT READY, but the first premise clears its three gates** on the 2026-09-12 basis; its out-of-sample evidence must be earned forward (ADR-0033) |
...
| 05  | Position sizing      | `copilot/risk/sizing`         | **Sizes with the stressed gap allowance** since 2026-09-12, and settled cash caps a cash account; both binding untested until live                |
...
the one that decides whether any of the rest is worth running.

**Refiled 2026-09-12, and this time the readings did move.** Two changes landed that each
rescale every R: sizing began carrying the playbook's stressed gap allowance, and the pinned
commission plan moved to Tiered because the account did. Every figure below is on that basis and
is **not comparable** to one filed before it. The AAPL holdout, re-scored, returns
`insufficient_evidence` - +0.017748 R over 112 trades, interval [-0.071442, +0.104593]. The
nine-symbol pool over the window every member covers is one fold of 137 trades at +0.011840 R,
interval [-0.086773, +0.088546]; the shifting-membership pool passes 20 of 38 folds at +0.014398
R per trade over 2,204 trades and **does not clear zero**, [-0.011206, +0.041472]. Of the
thirteen per-activation walk-forwards, eight pass their majority and **two** have intervals
clearing zero: AAPL next-close, [+0.001371, +0.085404], whose spent holdout still cannot; and
**SPY turn of the month, [+0.016124, +0.129851]**, whose interval did not clear zero before this
re-file. EEM, HYG and TLT have intervals wholly below zero.

**The turn of the month now passes all three of its declared gates** - 10 of 12 folds at
+0.074665 R, an interval clearing its predeclared bar of zero, and a null control at p 0.0419 -
and the gate that changed turned on **cost, not evidence**: the same 144 trades, cheaper
commission, and smaller positions paying less cost per R. Its null control percentile is
unchanged at 96.0, which is the correct result for a rank test whose seed and draws did not move.
It is still not a frozen candidate: its carved holdout is compromised rather than spendable
([ADR-0033](decisions/0033-the-turn-of-month-single-use-test-is-forward.md)), so the evidence
that would advance it is forward evidence on the paper clock.

**And attribution still finds no alpha anywhere**
...
sensitivity; EEM, HYG, SCHX and SPY signal-close are negative under both treatments; the turn of
the month reads `bracket_disagrees` with alpha -0.014327, and its gross +0.080925 R per trade is
+0.002250 cash, **+0.086909 factor exposure** and -0.008234 alpha; and the constant-membership
pool's gross +0.039437 is +0.000000 cash, +0.041586 factor exposure and -0.002149 alpha. Every
premise here is long the market and paid for it. Nothing is frozen, so stage 08's supervised
readiness has nothing to deploy.
...
| **Record a reassessment's cost basis in its own block** | 02, 04 | Opened 2026-09-12 by the re-file. `spend_holdout --reassess` re-scores a spent holdout and writes its trades, verdict and interval into the record, but the block carries **no `cost_model`**, so the number cannot be read back with the plan and sizing that produced it. That is the defect ADR-0025 fixed for verdicts, still open one level down, and it bit immediately: the AAPL re-score moved from +0.034010 R to +0.017748 R purely because the basis moved, and nothing in the block says so. The spend itself is fine - it keeps its original Fixed schedule and snapshot in its own block, so the single-use measurement is not conflated with a later re-score. Add the cost basis to the reassessment block, and note that `frozen_parameters_match_the_spend` now reads false for a good reason: the re-score's frozen set carries `gap_atr` and the 2026-09-05 spend's does not.                                                                                                       |
