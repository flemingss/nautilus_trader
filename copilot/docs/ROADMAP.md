...
Twenty-nine items. Grouped by blocking condition rather than by component, because that is
...
| **Wire alerting into the paths that need it**                                          | 08    | `live/alerting.py` exists and nothing calls it. The wiring is prep for the VM; delivery with real credentials is shaken out on the VM itself ([`DRAFT_PAPER_VM.md`](DRAFT_PAPER_VM.md), stage three). The playbook names the callers: an alert on *any order whose status cannot be confirmed* at monitoring end, safe mode's *preserve state and alert*, and the day command's stopping failures. Each wiring is small; the judgement is which conditions earn `CRITICAL`, because that severity wakes a human and feeds the kill switch ([ADR-0023](decisions/0023-a-critical-alert-demands-acknowledgement.md)).                                                                                                                                                                                                                                                                                                                                                                       |
...
| **Write the paper VM's bootstrap, services and environment template**                  | 08    | Target: ready before 2026-09-15, when the owner stands up a hand-configured Ubuntu 26.04 VM (8 vCPU, 32 GB, 150 GB); the plan and its stage checks are [`DRAFT_PAPER_VM.md`](DRAFT_PAPER_VM.md) ([ADR-0022](decisions/0022-the-always-on-host-is-a-dedicated-vm.md)). In `copilot/ops/` per [ADR-0006](decisions/0006-ops-progression.md) - roles in the repository, instances outside it: a bootstrap runbook drawn from *Standing up a new machine*, systemd units and Eastern-anchored timers for `day morning`, `day evening`, `day sweep` and the shakedown battery, an environment template naming every variable and no secret, clock synchronisation, log and record retention, and a host check that reports what a fresh VM is missing. The order-window shakedown moves to 10:45 so it cannot open with the sweep, which would cancel its probe. Size the VM for the source build: today's OOMs were on 15 GB.                                                                 |
...
| **A heartbeat, so silence is an alert**                                                | 09    | Pushover reports what the system sends; it cannot report that the system stopped sending. An unattended host that dies at 02:00 is indistinguishable from a quiet night. Needs a daily summary at `INFO` from the morning command and an external dead-man check that alerts on a missed one - the owner's cluster monitoring is the natural place, as an observer of the VM rather than its host.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **Review the guard's cooldown across a restart**                                       | 06    | Required by [ADR-0006](decisions/0006-ops-progression.md) before any unattended run and never done: `risk/guard.py` holds its breach state in memory and restores `ACTIVE` when a cooldown expires, so a process restart may end a cooldown early. Gateway's daily restart makes that the normal case rather than a corner. Decide whether breach state persists across restarts, and test it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **Make `day` safe to fire from a timer**                                               | 08    | Nothing in `day` stops a phase running on a weekend, a holiday, or twice for one session: a Saturday `day evening` prepares Monday, and Monday's prepares it again, filing two decision records for one session. A timer fires regardless, so each phase has to decide for itself - exit 0 with *nothing to do* when there is no session to act on, and refuse a second run for a session already recorded. Early closes move everything after 13:00. First item in [`DRAFT_PAPER_VM.md`](DRAFT_PAPER_VM.md)'s prep list.                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
