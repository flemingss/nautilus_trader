...
| `crates/live/src/python/node.rs`                               | Add a `risk_engine` getter to `PyLiveNode`                                                                                         | The handle has to come from somewhere. Mirrors the `cache` and `portfolio` getters line for line, including that it must be taken before a hosted run takes ownership of the node.                                                                                                                                                                                                                                                                                                                                                                          | 14 added lines, no existing line touched. **Upstream is churning this file** (+166/-88 since our base), so expect to re-read it at sync.                                                                                                                                                                     |
