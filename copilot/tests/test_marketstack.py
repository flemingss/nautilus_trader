...
No test here touches the network. The client is exercised against a stub urlopen so that
pagination and retry - the two behaviours whose failure modes are silent - are tested
deterministically rather than against whatever the provider does today.
...
def test_paging_continues_until_a_short_page(monkeypatch):
    ...
    Stopping at the first page would silently truncate a multi-year backfill to its most
    recent slice while still reporting success.
...
def test_a_bar_whose_close_is_outside_its_range_is_rejected():
    ...
    AAPL 2022-11-03 reports adj_close 138.65 under an adj_low of 138.75. A backtest fed
    that bar fills at a price the bar itself says never traded.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"close": "not-a-number"}, "unparsable price"),
